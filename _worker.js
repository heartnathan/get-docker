 ...
 * 構造回應
 * @param {any} body 回應體
 * @param {number} status 回應狀態碼
 * @param {Object<string, string>} headers 回應頭
 */
function makeRes(body, status = 200, headers = {}) {
    headers['access-control-allow-origin'] = '*' // 允許所有來源
    return new Response(body, { status, headers }) // 返回新構造的回應
}
...
function newUrl(urlStr, base) {
    try {
        ...
        return new URL(urlStr, base); // 嘗試構造新的URL對象
    } catch (err) {
        console.error(err);
...
<body>
<a href="https://github.com/heartnathan/get-docker" target="_blank" class="github-corner" aria-label="View source on Github">
...
<h1 class="title">Docker Hub 鏡像搜尋</h1>
<p class="subtitle">快速查詢、下載和部署 Docker 容器鏡像</p>
<div class="search-container">
<input type="text" id="search-input" placeholder="輸入關鍵詞搜尋鏡像，如: nginx, mysql, redis...">
<button id="search-button" title="搜尋">
<svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
<path d="M13 5l7 7-7 7M5 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round"></path>
</svg>
</button>
</div>
<p class="tips">基於 Cloudflare Workers / Pages 構建，利用全球邊緣網路實現毫秒級回應。</p>
</div>
<script>
...
export default {
    async fetch(request, env, ctx) {
        ...
        let checkHost; // 在這裡定義 checkHost 變量

        // 如果存在 ns 參數，優先使用它來確定 hub_host
        if (ns) {
            if (ns === 'docker.io') {
                hub_host = 'registry-1.docker.io'; // 設置上游地址為 registry-1.docker.io
            } else {
                hub_host = ns; // 直接使用 ns 作為 hub_host
            }
        } else {
        ...
        const fakePage = checkHost ? checkHost[1] : false; // 確保 fakePage 不為 undefined
        console.log`域名頭部: ${hostTop} 反代地址: ${hub_host} searchInterface: ${fakePage}`;
        ...
        // 新增：/v2/、/manifests/、/blobs/、/tags/ 先獲取token再請求
        if (
...
function httpHandler(req, pathname, baseHost) {
    ...
    reqHdrNew.delete("Authorization"); // 修復s3錯誤
